...
namespace Perfect_maze
{
    public partial class TBoard : UserControl
    {
        ...
        private List<TCell> bfsPath = new List<TCell>();
        public bool Reverse { get; set; } = false;
        public int BfsAnimStep = 0;
        public bool BfsAnimDone => BfsAnimStep >= bfsPath.Count;
        public TBoard()
        {
            InitializeComponent();
            DoubleBuffered = true;
        ...
        public void EscapePath()
        {
            ...
            bfsPath = goalCell != null ? TAlgorithm.GetPath(goalCell) : new List<TCell>();
            BfsAnimStep = 0;
            Invalidate();
        }
        public void Reset()
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (bfsPath.Count > 1)
            {
                ...
                for (int i = 1; i < BfsAnimStep; i++)
                {
                    var actCell = bfsPath[i];
                    var prevCell = bfsPath[i - 1];
                    var rc = new RectangleF();
                    if (actCell.Y == prevCell.Y)
                    {
                        rc.X = Math.Min(prevCell.X, actCell.X);
